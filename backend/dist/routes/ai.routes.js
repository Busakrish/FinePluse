...
router.get('/financial-twin', authenticate, (req, res) => {
...
router.get('/financial-health', authenticate, (req, res) => {
    const customerId = req.user?.customerId;
    if (!customerId)
        return res.status(400).json({ success: false, error: 'No customer profile.' });
    const twin = FinancialTwinManager.refreshTwin(customerId);
...
router.get('/stress', authenticate, (req, res) => {
    const customerId = req.user?.customerId;
    if (!customerId)
        return res.status(400).json({ success: false, error: 'No customer profile.' });
    const stress = FinancialStressEngine.evaluateStress(customerId);
...
router.get('/fraud-alerts', authenticate, (req, res) => {
    const customerId = req.user?.customerId;
    if (!customerId)
        return res.status(400).json({ success: false, error: 'No customer profile.' });
...
router.get('/next-best-action', authenticate, (req, res) => {
    const customerId = req.user?.customerId;
    if (!customerId)
        return res.status(400).json({ success: false, error: 'No customer profile.' });
