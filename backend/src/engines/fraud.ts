...
export interface FraudAnalysisResult {
  ...
  recommended_security_action: 'ALLOW' | 'MFA_CHALLENGE' | 'TEMPORARY_BLOCK' | 'FLAG_FOR_REVIEW';
}
...
export class FraudEngine {
  public static evaluateTransaction(
    ...
    const historicalTxns = db.filter(
      'transactions',
      (tx) => tx.customer_id === customerId && tx.type === 'DEBIT' && !tx.is_anomaly
    );
    ...
    let action: 'ALLOW' | 'MFA_CHALLENGE' | 'TEMPORARY_BLOCK' | 'FLAG_FOR_REVIEW' = 'ALLOW';
