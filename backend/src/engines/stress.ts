...
import { CustomerProfile, FinancialTwin, Loan } from '../db/types.js';
...
export interface StressAnalysisResult {
  ...
  score_breakdown: {
    emi_burden_penalty: number;
    savings_drop_penalty: number;
    missed_payments_penalty: number;
    liquid_buffer_penalty: number;
  };
}

export class FinancialStressEngine {
  public static evaluateStress(customerId: string): StressAnalysisResult {
