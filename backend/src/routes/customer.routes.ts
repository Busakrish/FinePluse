...
import { authenticate, requireCustomerIsolation, AuthenticatedRequest } from '../middleware/auth.js';
...
router.get('/:customerId', authenticate, requireCustomerIsolation, (req: AuthenticatedRequest, res: Response): any => {
...
router.get('/accounts/my', authenticate, (req: AuthenticatedRequest, res: Response): any => {
...
router.get('/goals/my', authenticate, (req: AuthenticatedRequest, res: Response): any => {
  const customerId = req.user?.customerId;
  if (!customerId) return res.status(400).json({ success: false, error: 'No customer profile.' });
...
router.post('/goals', authenticate, (req: AuthenticatedRequest, res: Response): any => {
  const customerId = req.user?.customerId;
  if (!customerId) return res.status(400).json({ success: false, error: 'No customer profile.' });
