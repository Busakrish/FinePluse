...
router.post('/simulate-journey', authenticate, (req: AuthenticatedRequest, res: Response): any => {
  const customerId = req.user?.customerId;
  if (!customerId) return res.status(400).json({ success: false, error: 'No customer profile.' });
  ...
  let guidance = '';
  let status: 'ELIGIBLE_RECOMMENDED' | 'CAUTION_MANAGEABLE' | 'BLOCKED_HIGH_STRESS' = 'ELIGIBLE_RECOMMENDED';
