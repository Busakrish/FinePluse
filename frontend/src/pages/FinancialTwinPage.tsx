    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 border-l-4 border-l-emerald-600 p-5 rounded-2xl shadow-xs space-y-2">
          ...
          <div className={`text-xl font-black ${(twin?.savings_growth_rate ?? 12) < 0 ? 'text-rose-700' : 'text-emerald-700'}`}>
            {(twin?.savings_growth_rate ?? 12) >= 0 ? '+' : ''}{twin?.savings_growth_rate ?? 12}%
          </div>
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-3xl space-y-4 shadow-xs">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
            ...
            <div className="text-lg font-black text-slate-900 mt-1">₹{(twin?.monthly_income ?? 85000).toLocaleString('en-IN')}</div>
          </div>
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
            <span className="text-[10px] text-slate-500 uppercase font-bold">Monthly Outflow</span>
            <div className="text-lg font-black text-slate-800 mt-1">₹{(twin?.monthly_expenses ?? 35800).toLocaleString('en-IN')}</div>
          </div>
          <div className="p-4 rounded-xl bg-amber-50/70 border border-amber-200">
            <span className="text-[10px] text-amber-800 uppercase font-bold">EMI Commitments</span>
            <div className="text-lg font-black text-amber-900 mt-1">₹{(twin?.monthly_emi_burden ?? 26500).toLocaleString('en-IN')}</div>
          </div>
          <div className="p-4 rounded-xl bg-emerald-50/70 border border-emerald-200">
            <span className="text-[10px] text-emerald-800 uppercase font-bold">Net Retained Savings</span>
            <div className="text-lg font-black text-emerald-800 mt-1">₹{(twin?.monthly_savings ?? 22700).toLocaleString('en-IN')}</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-3xl space-y-3 shadow-xs">
        ...
        <div className="space-y-2">
          {(behavioral?.insights?.length ? behavioral.insights : [
            'Regular payroll and trade income credits observed with verified bank ledger.',
            'Maintaining safe liquid reserves with positive month-over-month savings accumulation.'
