...
import { Send, QrCode, ArrowDownLeft, ShieldCheck, CheckCircle2, AlertTriangle, RefreshCw, Smartphone, ArrowRight, BotMessageSquare, Receipt, TrendingUp } from 'lucide-react';
      ...
      <div className="bg-white border border-slate-200 p-6 sm:p-8 rounded-3xl space-y-6 shadow-xs">
        ...
        {paymentResult && (
          <div className={`p-6 rounded-2xl border space-y-4 animate-in slide-in-from-bottom-2 ${
            ...
            {/* Post-Payment Direct Action Pathways */}
            <div className="pt-2 flex flex-wrap gap-2.5">
              <button
                onClick={() => navigate('/transactions')}
                className="flex-1 min-w-[140px] px-3.5 py-2 rounded-xl text-xs font-bold bg-white hover:bg-slate-50 text-slate-800 border border-slate-300 shadow-2xs flex items-center justify-center gap-1.5 transition"
              >
              ...
              >
                ...
                <span>Spending Coach</span>
                <ArrowRight className="w-3 h-3 text-slate-400" />
              </button>
              <button
                onClick={() => navigate('/assistant', { state: { prompt: `I just made a UPI payment of ₹${amount} for ${category}. How does this affect my monthly savings?` } })}
                className="flex-1 min-w-[160px] px-3.5 py-2 rounded-xl text-xs font-bold bg-blue-700 hover:bg-blue-800 text-white shadow-xs flex items-center justify-center gap-1.5 transition"
              >
                <BotMessageSquare className="w-3.5 h-3.5 text-blue-200" />
